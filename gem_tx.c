...
static inline void gem_mmio_write32(struct gem_priv *priv, uint32_t off,
                                    uint32_t val) {
  volatile uint32_t *reg = (volatile uint32_t *)((char *)priv->base_addr + off);
  *reg = rte_cpu_to_le_32(val);
  rte_wmb();
}

static inline uint32_t gem_mmio_read32(struct gem_priv *priv, uint32_t off) {
  volatile uint32_t *reg = (volatile uint32_t *)((char *)priv->base_addr + off);
...
static inline void gem_tx_kick(struct gem_priv *priv) {
  uint32_t ncr;
  ...
  ncr |= (GEM_NCR_MAN_PORT_EN | GEM_NCR_RXEN | GEM_NCR_TXEN | GEM_NCR_TXSTART);
...
static inline void gem_tx_cleanup(struct gem_queue *q) {
...
uint16_t gem_tx_pkt_burst(void *queue, struct rte_mbuf **bufs,
                          uint16_t nb_bufs) {
  ...
  while (nb_tx < nb_bufs) {
    ...
    uint64_t iova;
    gem_dma_invalidate(bd, sizeof(*bd));
    GEM_LOG("TX cleanup: idx=%u ctrl=0x%08x USED=%u", q->tx_tail, bd->ctrl,
            !!(bd->ctrl & GEM_TX_USED));
    if ((bd->ctrl & GEM_TX_USED) == 0)
      break;
    ...
    if (unlikely(m->nb_segs != 1)) {
      ...
      if ((++log_throttle & 0xFFFFu) == 0)
        GEM_LOG("TX drop: multi-seg packet not supported (nb_segs=%u)",
                m->nb_segs);
      nb_tx++;
      continue;
    }
    ...
    if (unlikely(m->pkt_len == 0 || m->pkt_len > GEM_TX_LEN_MASK)) {
      ...
      priv->tx_errs++;
      if ((++log_throttle & 0xFFFFu) == 0)
        GEM_LOG("TX drop: bad len=%u (max=%u)", m->pkt_len,
                (unsigned)GEM_TX_LEN_MASK);
      nb_tx++;
      continue;
    }
    ...
    q->tx_sw_ring[q->tx_head] = m;
    {
      ...
      GEM_LOG("TX mbuf va=%p iova=0x%llx", rte_pktmbuf_mtod(m, void *),
              (unsigned long long)iova);
    }
    ...
    GEM_LOG("TX[%u]: bd@%p addr_iova=0x%08x payload_va=%p payload_iova=0x%llx "
            "len=%u ctrl=0x%08x",
            desc_idx, (void *)bd, bd->addr, rte_pktmbuf_mtod(m, void *),
            (unsigned long long)iova, m->pkt_len, ctrl);

    GEM_LOG("TX[%u]: ctrl=0x%08x addr=0x%08x len=%u", desc_idx, ctrl, bd->addr,
            m->pkt_len);
